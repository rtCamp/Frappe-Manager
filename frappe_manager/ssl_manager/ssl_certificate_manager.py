...
from frappe_manager.utils.helpers import create_symlink, format_ssl_certificate_time_remaining
...
class SSLCertificateManager:
    ...
    def renew_certificate(self, recertificate: RenewableSSLCertificate):
        if self.needs_renewal(recertificate):
            ...
            time_remaining_txt = format_ssl_certificate_time_remaining(recertificate.expiry)
            richprint.print(
                f"[yellow]{recertificate.domain}[/yellow] certificate is still valid for {time_remaining_txt}."
            )
