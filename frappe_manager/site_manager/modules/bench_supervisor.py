...
class BenchSupervisor:
    ...
    def switch_bench_env(self, service: str = 'frappe', timeout: int = 30, interval: int = 1) -> None:
        ...
        for _ in range(timeout):
            try:
                self.docker_client.compose.exec(
                    service=service, command=f"test -e {socket_path}", user='frappe', stream=False
                )
                break
            except DockerException as e:
                time.sleep(interval)
            ...
            raise BenchOperationException(
                self.bench_name, message=f'Supervisor socket for frappe service not created after {timeout} seconds'
            )
