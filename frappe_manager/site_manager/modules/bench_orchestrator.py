...
class BenchOrchestrator:
    ...
    def create_bench(self, is_template_bench: bool = False):
        ...
        try:
            ...
            output = bench.docker_client.compose.up(
                services=[], detach=True, pull="never",
                force_recreate=True, stream=bench.quiet
            )
            if bench.quiet:
            ...
            # Create bench site
            richprint.change_head(f"Creating bench site {bench.name}")
            bench.site_manager.create_bench_site()
            richprint.print(f"Created bench site {bench.name}")
    ...
    def _create_template_bench(self):
        ...
        bench.save_bench_config()
        richprint.print(f"Created template bench: {bench.name}", emoji_code=":white_check_mark:")
    ...
    def _remove_archived_directory(self):
        ...
        try:
            ...
            raise BenchOperationException(
                bench.name,
                "Failed to remove /workspace/frappe-bench/archived directory."
            )
    ...
    def _handle_creation_failure(self, exception: Exception):
        ...
        richprint.stop()
        richprint.error(f"[red][bold]Error Occured: [/bold][/red]{exception}")
    ...
    def start_bench(
        ...
        if bench.workers.compose_file_manager.exists():
            ...
            output = bench.workers.docker_client.compose.up(
                services=[], detach=True, pull="never",
                force_recreate=force, stream=bench.quiet
            )
            if bench.quiet:
                richprint.live_lines(output, padding=(0, 0, 0, 2))
    ...
    def update_alias_domains(
        ...
        if bench.name in add_list:
            richprint.warning(f"Skipping '{bench.name}' - primary domain cannot be added as alias.")
            add_list = [d for d in add_list if d != bench.name]
        
        if bench.name in remove_list:
            richprint.stop()
            raise ValueError(
                f"Cannot remove primary domain '{bench.name}'. Only alias domains can be removed."
            )
