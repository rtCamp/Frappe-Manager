...
from frappe_manager.services_manager.services_exceptions import DatabaseServiceDBCreateFailed, DatabaseServiceDBExportFailed, DatabaseServiceDBImportFailed, DatabaseServiceDBNotFoundError, DatabaseServiceDBRemoveFailError, DatabaseServiceException, DatabaseServicePasswordNotFound, DatabaseServiceStartTimeout, DatabaseServiceUserRemoveFailError
...
class MariaDBManager(DatabaseServiceManager):
    def __init__(self, database_server_info: DatabaseServerServiceInfo, compose_project: ComposeProject, run_on_compose_service: Optional[str] = None) -> None:
        """
        Database manager
        """
    ...
    def db_run_query(self, query: str, raise_exception_obj: Optional[DatabaseServiceException] = None, capture_output: bool = False):
        ...
        try:
            output = self.compose_project.docker.compose.exec(self.run_on_compose_service, command=db_query, stream=not capture_output)
            if capture_output:
                return output
    ...
    def is_db_running(self) -> bool:
        ...
        try:
            output = self.compose_project.docker.compose.exec(self.run_on_compose_service, command=db_started_command, stream=False)
    ...
    def get_db_users(self) -> Dict[str,str]:
        ...
        output: SubprocessOutput = self.db_run_query(show_db_user_command, raise_exception_obj=exception,capture_output=True)
    ...
    def get_all_databases(self) -> List[str]:
        ...
        db_exits_exception = DatabaseServiceException(self.database_server_info.host, 'Failed to get list of all databases.')
    ...
    def grant_user_privilages(self, db_user:str, db_name: str):
        ...
        grant_user_exception = DatabaseServiceException(self.database_server_info.host,f'Failed to grant prvilages for user {db_user} on {db_name}.')
    ...
    def add_user(self, db_user: str, db_pass: str, db_user_host: str = '%', force: bool = False, timeout = 25):
        if self.check_user_exists(db_user, db_user_host):
            if force:
                ...
                raise DatabaseServiceException(self.run_on_compose_service,f'User {db_user} for {db_user_host} already exists.')
    ...
    def db_export(self, db_name: str, export_file_path: Union[str,Path]):
        ...
        try:
            output = self.compose_project.docker.compose.exec(self.run_on_compose_service, command=db_export_command, stream=False)
    ...
    def db_import(self, db_name: str, host_db_file_path: Path, force: bool = False):
        ...
        try:
            ...
            output = self.compose_project.docker.compose.exec(self.run_on_compose_service, command=db_import_command, stream=False)
        except DockerException:
