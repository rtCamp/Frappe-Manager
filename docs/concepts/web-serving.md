...
- **`X-Forwarded-Proto`.** The global proxy sets it to the scheme it terminated, and passes an inbound value through untouched when one is already present (so a CDN's value wins). The bench nginx forwards that same value verbatim. Frappe reads the header directly, not the WSGI scheme: `frappe.utils.get_url()` treats exactly the value `https` as HTTPS and anything else as HTTP, and that decides every absolute URL it builds (email links, redirects, OAuth callbacks). Setting `host_name` in the site config overrides all of it.
