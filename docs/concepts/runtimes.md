...
stateDiagram-v2
    ...
    mount --> image : fm switch BENCH TAG\n(migrates the site onto the image;\ndata carries over)
    image --> mount : fm update --runtime mount\n(materializes a workspace from the\ndeployed image; no migrate needed)
    mount --> mount : fm bake -> new TAG
    image --> image : fm switch TAG / --previous
```

Both directions preserve your site and database:

- **mount → image**: `fm switch <bench> <tag>` runs the full deploy pipeline - the site is migrated onto the image and the workspace stops being the source of truth.
