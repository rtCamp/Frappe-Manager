...
| `restart` | **The host-side owner of the drain/graceful-restart orchestration** (§8.3): sequences maintenance → **drain via in-container `fmx` (called by `exec`)** → `compose restart`/recreate → resume — **compose for both runtimes**. `switch` is literally this restart **+ a new image tag + migrate**. Supervisor-level restart is the default (no flag); `--container`/`--rolling` select the mode, `--web`/`--workers` the service groups. |
