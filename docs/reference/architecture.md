...
- **`image`**: app code is baked into an immutable image (`fm bake`) and deploys happen by switching images (`fm switch`). Only mutable data is bound in from the workspace: the site directory, `common_site_config.json`, `apps.txt`, `logs` and `config`.
