...
Every migration backs up what it is about to touch, per bench and per version, before that version's steps run. The layout on disk, restoring by hand, and the troubleshooting recipes live in [Backup & Restore](../guides/backup-restore.md#before-a-migration); version-specific extra artifacts are listed in the [Migration History](migration-history.md#version-backups).
