...
| `drain_timeout` | `300` | seconds to wait for **one in-flight job**; restart, stop, update and switch abort when exceeded |
| `queue_drain_timeout` | `1800` | seconds to wait for the whole **queue backlog** to empty before a redis queue move; `0` = no limit |
...
**Queued jobs are the one thing that cannot be regenerated, so the queue side is gated.** Before moving the queue, FM counts what is still pending on the server being left; if anything is, it pauses the producers (Frappe's `maintenance_mode`, which also makes the scheduler inactive, plus the nginx 503) and waits for the workers to clear the backlog, so there is nothing to migrate. If the backlog outlasts `[workers].queue_drain_timeout` (30 minutes by default, `0` for no limit) nothing is changed, producers resume, and `--abandon-queued` is the way through -- it switches anyway and those jobs are never run. An empty queue needs no window and switches straight through.

The cache and realtime are deliberately NOT gated: the cache holds derived data (doctype meta, table columns) that is regenerated on the first miss and *wants* to be cold after a cutover, and realtime is pub/sub with nothing at rest. FM never copies redis data in either direction, because a copy would carry the old server's worker-registry keys (workers that no longer exist, jobs recorded as started by them), and the transports that could do it (`SYNC`, `MIGRATE`, `DEBUG`) are exactly what managed providers block. Recent job history in `rq:failed`/`rq:finished` (what the RQ Job doctype reads) does not survive a queue cutover either.
