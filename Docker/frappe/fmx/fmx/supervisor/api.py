...
def get_service_names() -> List[str]:
    ...
    services = sorted([file.stem for file in FM_SUPERVISOR_SOCKETS_DIR.glob("*.sock") if file.is_socket()])
...
def start_service(
    ...
    try:
        ...
        logger.info(
            f"Start service completed: service={service_name}, started={started_count}, already_running={already_running_count}, failed={failed_count}"
        )
        return result
    except SupervisorError as e:
...
def restart_service(
    ...
    try:
        ...
        failed_count = len(result.get("failed", [])) if result else 0
        logger.info(
            f"Restart service completed: service={service_name}, stopped={stopped_count}, started={started_count}, failed={failed_count}"
        )
        return result
    except SupervisorError as e:
...
def signal_service(service_name: str, signal_name: str, process_name_list: Optional[List[str]] = None) -> bool:
    """
