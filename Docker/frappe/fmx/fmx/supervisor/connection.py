...
def check_supervisord_connection(service_name: str) -> ServerProxy:
    ...
    try:
        ...
        raise SupervisorConnectionError(
            f"XML-RPC Fault during connection check: {e.faultString}", service_name=service_name, original_exception=e
        )
    except ProtocolError as e:
        raise SupervisorConnectionError(
            f"Protocol error during connection check: {e}", service_name=service_name, original_exception=e
        )
    except (OSError, IOError, ConnectionRefusedError, socket.error, socket.timeout) as e:
        raise SupervisorConnectionError(
            f"Socket/Connection error during connection check: {e}", service_name=service_name, original_exception=e
        )
    except Exception as e:
        raise SupervisorConnectionError(
            f"Unexpected error during connection check: {e}", service_name=service_name, original_exception=e
        )
