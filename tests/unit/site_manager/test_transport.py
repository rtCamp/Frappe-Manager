...
construction from RemoteConfig, and env-substitution of registry creds.
"""
...
from frappe_manager.site_manager.bench_config import RemoteConfig
...
def test_build_docker_host_uses_remote_config_user_port():
    rc = RemoteConfig(ssh_server="ignored", ssh_user="deploy", ssh_port=2200)
...
def test_remote_docker_host_from_config():
    rc = RemoteConfig(ssh_server="prod.example", ssh_user="frappe", ssh_port=22)
...
def test_remote_docker_host_none_when_no_server():
    ...
    assert remote_docker_host(RemoteConfig()) is None
