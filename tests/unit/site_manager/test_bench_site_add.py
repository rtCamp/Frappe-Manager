...
`bench_config.db_name` names the FIRST site's mariadb schema. Reusing it for a second site would
...
def manager(captured):
    ...
    m.bench_config.get_database_config.return_value = None  # mariadb, not external
...
def test_the_database_entry_is_looked_up_for_the_named_site(manager):
    """A bench can hold one site on mariadb and another on an external server, so the lookup has
...
def test_an_external_added_site_sends_no_mariadb_root_password(manager, captured):
    """The mariadb root password means nothing on a server fm does not own and must never be sent
