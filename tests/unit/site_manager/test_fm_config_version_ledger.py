...
ledger is read BEFORE any migration can run: without the seed a v0.20 host reads 0.0.0 and
discovery re-selects the frozen v0.19/v0.20 migrations against it. The write path owns the
...
class TestLedgerSeeding:
    ...
    def test_pre_rename_spelling_reads_as_the_ledger(self, tmp_path):
        ...
        assert config.get_system_migration_version() == Version("0.20.0")
    ...
    def test_migrated_to_wins_over_both_legacy_spellings(self, tmp_path):
        ...
        assert config.get_system_migration_version() == Version("0.21.0")
    ...
    def test_old_spelling_wins_over_the_retired_version_key(self, tmp_path):
        ...
        path = _config(tmp_path, 'version = "0.21.0"\n[migration_state]\nsystem_migrated_to = "0.20.0"\n')
...
class TestDiskCutover:
    ...
    def test_first_stamp_on_a_legacy_file_leaves_exactly_one_version_key(self, tmp_path):
        path = _config(tmp_path, 'version = "0.20.0"\n[migration_state]\nsystem_migrated_to = "0.20.0"\n')

        config = FMConfigManager.import_from_toml(path)
        config.set_system_migration_version(Version("0.21.0"))

        text = path.read_text()
        assert 'migrated_to = "0.21.0"' in text
    ...
    def test_an_ordinary_save_retires_the_version_key_but_never_invents_a_ledger(self, tmp_path):
        ...
        assert 'migrated_to = "0.20.0"' in text
        # And a reload agrees with what the original file meant.
        assert FMConfigManager.import_from_toml(path).get_system_migration_version() == Version("0.20.0")
...
class TestExportPathDefault:
    def test_the_setter_saves_to_the_file_the_config_was_loaded_from(self, tmp_path, monkeypatch):
        ...
        assert 'migrated_to = "0.21.0"' in path.read_text()
