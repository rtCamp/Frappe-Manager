...
#   test            Run migration with FM_MIGRATION_REPO
...
#   FM_MIGRATION_SSH          SSH target (e.g., frappe@myserver.example.com)
#   FM_MIGRATION_BENCH           Bench/site name (e.g., mybench.example.com)
#   FM_MIGRATION_HOME            Home on remote (e.g., /home/frappe)
#   FM_MIGRATION_UV              Path to uv binary (e.g., /home/frappe/.local/bin/uv)
#   FM_MIGRATION_PREVIOUS_PYTHON Python for previous FM (e.g., 3.12)
#   FM_MIGRATION_PYTHON          Python for target FM (e.g., 3.13)
#   FM_MIGRATION_PREVIOUS_REPO   Previous version spec (e.g., git+https://...@fix/install-deps)
#   FM_MIGRATION_REPO            Target version spec (e.g., git+https://...@fix-migrations)
#   FM_MIGRATION_FLAGS   Migration flags (e.g., all --yes)
#   FM_MIGRATION_STOP_CONTAINERS Stop FM containers before migration (default: true)
#
...
: "${FM_MIGRATION_SSH:?FM_MIGRATION_SSH is not set. Create .env or export it.}"
: "${FM_MIGRATION_BENCH:?FM_MIGRATION_BENCH is not set. Create .env or export it.}"
: "${FM_MIGRATION_HOME:?FM_MIGRATION_HOME is not set. Create .env or export it.}"
: "${FM_MIGRATION_UV:?FM_MIGRATION_UV is not set. Create .env or export it.}"
: "${FM_MIGRATION_PREVIOUS_PYTHON:?FM_MIGRATION_PREVIOUS_PYTHON is not set. Create .env or export it.}"
: "${FM_MIGRATION_PYTHON:?FM_MIGRATION_PYTHON is not set. Create .env or export it.}"
: "${FM_MIGRATION_PREVIOUS_REPO:?FM_MIGRATION_PREVIOUS_REPO is not set. Create .env or export it.}"
: "${FM_MIGRATION_REPO:?FM_MIGRATION_REPO is not set. Create .env or export it.}"
: "${FM_MIGRATION_FLAGS:?FM_MIGRATION_FLAGS is not set. Create .env or export it.}"
: "${FM_MIGRATION_STOP_CONTAINERS:=true}"   # Set to false to skip stopping containers before migration

SSH_OPTS="-o ConnectTimeout=5 -o BatchMode=yes"
SSH="ssh $SSH_OPTS $FM_MIGRATION_SSH"
FM_MIGRATION_DIR="$FM_MIGRATION_HOME/frappe"           # Working FM data directory
FM_MIGRATION_TMP="$FM_MIGRATION_HOME/.fm-tmp"          # Transient venv for building
...
check_server() {
    if ! ssh $SSH_OPTS "$FM_MIGRATION_SSH" "echo ok" 2>/dev/null; then
        err "Server unreachable: $FM_MIGRATION_SSH"
        echo "  Check:"
        echo "    • Are you connected to the VPN?"
        echo "    • Is the server hostname correct?"
        echo "    • Try: ssh $FM_MIGRATION_SSH"
        exit 1
    fi
    ok "Server reachable: $FM_MIGRATION_SSH"
}
...
install_fm() {
    ...
    if ! ssh_cmd "rm -rf \"$FM_MIGRATION_TMP\" && \"$FM_MIGRATION_UV\" venv --python \"$python_ver\" \"$FM_MIGRATION_TMP\""; then
        echo "  ${RED}✗${NC} Failed to create venv (is uv installed at $FM_MIGRATION_UV?)" >&2
        exit 1
    fi
    echo "  ${GREEN}✓${NC} Venv created at $FM_MIGRATION_TMP" >&2

    if ! ssh_cmd "\"$FM_MIGRATION_UV\" pip install --python \"$FM_MIGRATION_TMP/bin/python\" \"$repo\" --reinstall --refresh --quiet"; then
    ...
    local version
    version=$(ssh_cmd "$FM_MIGRATION_TMP/bin/python -c 'from importlib.metadata import version; print(version(\"frappe-manager\"))'" 2>/dev/null)
    if [[ -z "$version" ]]; then
...
finalize_install() {
    local version="$1"
    local target="$FM_MIGRATION_HOME/fm.$version"

    echo "  ${CYAN}⋮${NC} Moving $FM_MIGRATION_TMP → $target" >&2
    ssh_cmd "mkdir -p \"$target\""
    # Merge venv contents into target
    ssh_cmd "cp -a \"$FM_MIGRATION_TMP/.\" \"$target/\" && rm -rf \"$FM_MIGRATION_TMP\""
    # Fix shebangs and venv config so the relocation is transparent
    ssh_cmd "if [ -d \"$target/bin\" ]; then
        for f in \"$target/bin\"/*; do
            [ -f \"\$f\" ] && sed -i \"s|$FM_MIGRATION_TMP|$target|g\" \"\$f\" 2>/dev/null || true
        done
    fi"
    ssh_cmd "if [ -f \"$target/pyvenv.cfg\" ]; then
        sed -i \"s|$FM_MIGRATION_TMP|$target|g\" \"$target/pyvenv.cfg\"
    fi"
...
cmd_init() {
    ...
    local ref
    ref=$(parse_ref "$FM_MIGRATION_PREVIOUS_REPO")
    info "Source ref: $ref"

    # ── Step 0: Clean existing ~/frappe ─────────────────────────────────
    echo ""
    if ssh_cmd "test -d \"$FM_MIGRATION_DIR\"" 2>/dev/null; then
        info "Removing existing ~/frappe …"
        ssh_cmd "rm -rf \"$FM_MIGRATION_DIR\""
        ok "Cleaned ~/frappe"
    fi

    # ── Step 1-3: Install previous version ────────────────────────────────
    local version
    version=$(install_fm "$FM_MIGRATION_PREVIOUS_REPO" "$FM_MIGRATION_PREVIOUS_PYTHON")
    ok "Previous version: $version"

    # ── Step 4: Start global services (mariadb, nginx-proxy) ──────────────
    echo ""
    info "Starting global services …"
    ssh_cmd "cd \"$FM_MIGRATION_HOME\" && FRAPPE_MANAGER_HOME=\"$FM_MIGRATION_DIR\" \"$FM_MIGRATION_TMP/bin/fm\" services start all" || {
    ...
    # ── Step 5: Create bench ──────────────────────────────────────────────
    echo ""
    info "Creating bench: $FM_MIGRATION_BENCH …"
    ssh_cmd "cd \"$FM_MIGRATION_HOME\" && FRAPPE_MANAGER_HOME=\"$FM_MIGRATION_DIR\" \"$FM_MIGRATION_TMP/bin/fm\" create \"$FM_MIGRATION_BENCH\"" || {
        err "fm create failed"
        exit 1
    }
    ok "Bench created: $FM_MIGRATION_BENCH"

    # ── Step 6: Stop all FM containers (optional) ─────────────────────────
    echo ""
    if [[ "$FM_MIGRATION_STOP_CONTAINERS" == "true" ]]; then
        info "Stopping all FM containers …"
        ssh_cmd "docker ps --filter name=fm_ -q | xargs -r docker stop" || true
        ok "FM containers stopped"
    else
        info "Skipping container stop (FM_MIGRATION_STOP_CONTAINERS=false)"
    fi
    ...
    ssh_cmd "mkdir -p \"$target\" && mv \"$FM_MIGRATION_DIR\" \"$target/frappe\""
    ...
    echo ""
    header "Init Summary"
    echo "  Server:      $FM_MIGRATION_SSH"
    echo "  Version:     $version"
    echo "  Location:    $target"
    echo "  Bench:       $FM_MIGRATION_BENCH"
    echo "  FM binary:   $target/bin/fm"
    echo "  Data:        $target/frappe/"
    echo "  SITE_DIR:    $target/frappe/sites/$FM_MIGRATION_BENCH/"
    echo ""
...
cmd_setup() {
    ...
    if [[ -z "$version" ]]; then
        ...
        version=$(ssh_cmd "ls -1d \"$FM_MIGRATION_HOME/fm.\"* 2>/dev/null | while read d; do
            [ -d \"\$d/frappe\" ] && basename \"\$d\" | sed 's/^fm.//'
        done | sort -V | tail -1")
        if [[ -z "$version" ]]; then
            err "No versioned backups found in $FM_MIGRATION_HOME/fm.*/"
    ...
    local source="$FM_MIGRATION_HOME/fm.$version/frappe"
    ...
    echo ""
    info "Restoring $source → $FM_MIGRATION_DIR"
    ssh_cmd "rm -rf \"$FM_MIGRATION_DIR\" && cp -a \"$source\" \"$FM_MIGRATION_DIR\""
    ok "Restored fm.$version/frappe → ~/frappe"

    echo ""
    header "Setup Complete"
    echo "  Restored:    fm.$version/frappe"
    echo "  To:          $FM_MIGRATION_DIR"
    echo "  Bench:       $FM_MIGRATION_BENCH"
    echo "  Sites:       $FM_MIGRATION_DIR/sites/$FM_MIGRATION_BENCH/"
    echo ""
...
# ─── test ───────────────────────────────────────────────────────────────────────
cmd_test() {
    header "Test — Run migration with FM_MIGRATION_REPO"

    local ref
    ref=$(parse_ref "$FM_MIGRATION_REPO")
    info "Target ref: $ref"
    ...
    local version
    version=$(install_fm "$FM_MIGRATION_REPO" "$FM_MIGRATION_PYTHON")
    ...
    ssh_cmd "ln -sf \"fm.$version\" \"$FM_MIGRATION_HOME/fm-active\""
    ok "Symlink: fm-active → fm.$version"

    # ── Step 5: Run migration ───────────────────────────────────────────
    echo ""
    info "Running: fm migrate $FM_MIGRATION_FLAGS"
    echo ""

    # Run migration in foreground with live output streaming
    ssh_cmd "cd \"$FM_MIGRATION_DIR\" && FRAPPE_MANAGER_HOME=\"$FM_MIGRATION_DIR\" \"$FM_MIGRATION_HOME/fm-active/bin/fm\" migrate $FM_MIGRATION_FLAGS 2>&1" \
        || warn "Migration exited with code $?"
    echo ""

    # ── Results ─────────────────────────────────────────────────────────
    header "Migration Results"
    echo "  Previous:    $FM_MIGRATION_PREVIOUS_REPO"
    echo "  Target:      $ref"
    echo "  Version:     $version"
    echo "  Location:    $FM_MIGRATION_HOME/fm.$version"
    echo "  Symlink:     fm-active → fm.$version"
    echo "  Data:        $FM_MIGRATION_DIR/        (migrated)"
    echo ""
...
cmd_full() {
    ...
    echo ""
    echo "  └─ Step 2/2: Run migration against FM_MIGRATION_REPO …"
    echo ""
    cmd_test
...
cmd_status() {
    ...
    local SITE_DIR="$FM_MIGRATION_DIR/sites/$FM_MIGRATION_BENCH"
    ...
    local SDIR="$FM_MIGRATION_DIR/services"
    ...
    ssh_cmd "ls -la \"$FM_MIGRATION_DIR/\" 2>/dev/null | sed 's/^/    /' || echo '    FM dir: not found'"

    # ── Versioned backups ────────────────────────────────────────────────
    echo ""
    echo "  ── Versioned backups ────────────────────────────────────────"
    ssh_cmd "for d in \"$FM_MIGRATION_HOME/fm.\"*; do
        if [ -d \"\$d/frappe\" ]; then
...
cmd_diff() {
    ...
    local prev_dir
    prev_dir=$(ssh_cmd "ls -1d \"$FM_MIGRATION_HOME/fm.\"* 2>/dev/null | while read d; do
        [ -d \"\$d/frappe\" ] && echo \"\$d\"
    done | sort -V | tail -1")

    if [[ -z "$prev_dir" ]]; then
        err "No versioned init-state backups found in $FM_MIGRATION_HOME/fm.*/"
    ...
    local prev_dc="$prev_dir/frappe/sites/$FM_MIGRATION_BENCH/docker-compose.yml"
    local curr_dc="$FM_MIGRATION_DIR/sites/$FM_MIGRATION_BENCH/docker-compose.yml"

    echo "  Previous:  $FM_MIGRATION_SSH:$prev_dc  ($prev_name)"
    echo "  Current:   $FM_MIGRATION_SSH:$curr_dc"
    echo ""
...
cmd_perms() {
    ...
    echo "  FM path: $FM_MIGRATION_DIR"
    echo ""

    local paths=(
        "$FM_MIGRATION_DIR"
        "$FM_MIGRATION_DIR/fm_config.toml"
        "$FM_MIGRATION_DIR/services/docker-compose.yml"
        "$FM_MIGRATION_DIR/sites/$FM_MIGRATION_BENCH/docker-compose.yml"
        "$FM_MIGRATION_DIR/sites/$FM_MIGRATION_BENCH/bench_config.toml"
    )
...
cmd_logs() {
    ...
    ssh_cmd "tail -100 \"$FM_MIGRATION_DIR/logs/fm.log\" 2>/dev/null || tail -100 \"$FM_MIGRATION_DIR/fm.log\" 2>/dev/null || echo '    fm.log not found under $FM_MIGRATION_DIR/'" | sed 's/^/    /'
}
...
cmd_versions() {
    ...
    echo ""
    if ssh_cmd "test -L \"$FM_MIGRATION_HOME/fm-active\"" 2>/dev/null; then
        local target
        target=$(ssh_cmd "readlink \"$FM_MIGRATION_HOME/fm-active\"")
...
cmd_switch() {
    ...
    if ssh_cmd "test -d \"$FM_MIGRATION_HOME/fm.$version\"" 2>/dev/null; then
        ssh_cmd "ln -sf \"fm.$version\" \"$FM_MIGRATION_HOME/fm-active\""
        ok "Symlink updated: fm-active → fm.$version"
    else
        err "Directory fm.$version not found in $FM_MIGRATION_HOME"
        echo "  Available versions:"
        ssh_cmd "ls -1d \"$FM_MIGRATION_HOME/fm.\"* 2>/dev/null | sed 's/^/    /'"
        exit 1
    fi
}
...
cmd_cleanup() {
    ...
    if ssh_cmd "test -d \"$FM_MIGRATION_TMP\"" 2>/dev/null; then
        ssh_cmd "rm -rf \"$FM_MIGRATION_TMP\""
        ok "Removed temp venv: $FM_MIGRATION_TMP"
        cleaned=true
    fi
    ...
    while read line; do
        ok "$line"
        cleaned=true
    done < <(ssh_cmd "for d in \"$FM_MIGRATION_HOME/.fm-tmp-\"*; do
...
main() {
    ...
    if [[ -z "$cmd" ]]; then
        ...
        echo "  test            Run migration with FM_MIGRATION_REPO"
        ...
        echo ""
        echo "Environment:"
        echo "  FM_MIGRATION_SSH=$FM_MIGRATION_SSH"
        echo "  FM_MIGRATION_BENCH=$FM_MIGRATION_BENCH"
        echo "  FM_MIGRATION_HOME=$FM_MIGRATION_HOME"
        echo "  FM_MIGRATION_PREVIOUS_REPO=$FM_MIGRATION_PREVIOUS_REPO"
        echo "  FM_MIGRATION_REPO=$FM_MIGRATION_REPO"
        exit 1
    fi
