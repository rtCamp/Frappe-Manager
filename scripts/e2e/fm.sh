...
CreateSite() {
	...
	if [ ! "${envN:-}" ]; then
		fm create "$siteName" --non-interactive || {
			info_red "Failed to create site $siteName"
			exit 1
		}
	else
		fm create "$siteName" --environment "$envN" --non-interactive || {
...
DeleteSite() {
	...
	fm delete "$siteName" --yes --non-interactive
}

GetInfoSite() {
	local siteName="$1"
	echo "Info SiteName: $siteName"
	fm info "$siteName" --non-interactive
}

ListSites() {
	echo "List Sites"
	fm list --non-interactive
}

StartSite() {
	local siteName="$1"
	echo "Start SiteName: $siteName"
	fm start "$siteName" --non-interactive
}

StopSite() {
	local siteName="$1"
	echo "Stop SiteName: $siteName"
	fm stop "$siteName" --non-interactive
}
