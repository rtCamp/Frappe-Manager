...
if [[ ! "$(docker images -q frappe-local-nginx)" == "" ]]; then
    ...
    docker build --load -t frappe-local-nginx docker-images/nginx-docker/.
fi
...
if [[ ! "$(docker images -q frappe-local-mailhog)" == "" ]]; then
    ...
    docker build --load -t frappe-local-mailhog docker-images/mailhog-docker/.
fi
...
if [[ ! "$(docker images -q frappe-local-rqdashboard)" == "" ]]; then
    ...
    docker build --load -t frappe-local-rqdashboard docker-images/rq-dashboard-docker/.
fi
...
if [[ ! "$(docker images -q frappe-local-frappe)" == "" ]]; then
    ...
    docker build --load -t frappe-local-frappe docker-images/frappe-docker/.
fi
